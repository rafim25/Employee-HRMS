...
async function getDataPegawai() {
  try {
    ...
    const data_nama_pegawai = resultDataPegawai.map(
...
async function getDataKehadiran() {
  try {
    ...
    dataKehadiran.forEach((kehadiran) => {
      const { nik, bulan, jenis_kelamin, nama_jabatan, hadir, sakit, alpha } =
        kehadiran;
      const nama_pegawai =
        data_nama_pegawai.find((nama) => nama === kehadiran.nama_pegawai) ||
        "-";
      const nik_pegawai = data_nik.find((nik) => nik === kehadiran.nik) || "-";
      dataKehadiranMap.set(nik_pegawai, {
        nama_pegawai,
        bulan,
        jenis_kelamin,
        nama_jabatan,
    ...
    dataKehadiranMap.forEach(
      (
        { nik, bulan, jenis_kelamin, nama_jabatan, hadir, sakit, alpha },
        nikPegawai
      ) => {
        const nama_pegawai =
          data_nama_pegawai.find(
            (nama) => nama === dataKehadiranMap.get(nikPegawai).nama_pegawai
          ) || "-";
        resultDataKehadiran.push({
          nama_pegawai,
          nik,
          bulan,
          jenis_kelamin,
...
async function getDataJabatan() {
  ...
  try {
    dataJabatan.forEach((jabatan) => {
      ...
      datajabatanMap.set(nama_jabatan, {
        gaji_pokok,
        tj_transport,
        uang_makan,
      });
    ...
    datajabatanMap.forEach(
      ({ gaji_pokok, tj_transport, uang_makan }, nama_jabatan) => {
        resultDatajabatan.push({
          nama_jabatan,
          gaji_pokok,
          tj_transport,
          uang_makan,
